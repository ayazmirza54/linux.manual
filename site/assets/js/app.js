...
(function () {
  ...
  var store = {
    get: function (k, fallback) {
      try {
        var v = localStorage.getItem('linuxmanual.' + k);
        return v === null ? fallback : JSON.parse(v);
      } catch (e) { return fallback; }
    },
    set: function (k, v) {
      try { localStorage.setItem('linuxmanual.' + k, JSON.stringify(v)); } catch (e) { /* private mode */ }
    }
  };
  ...
  function paint(html, opts) {
    ...
    document.title = (opts.title ? opts.title + ' — ' : '') + 'linux.manual';
  }
